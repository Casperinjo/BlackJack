...
namespace BlackJack
{
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            double slutSumma = 0;
            int wager = 0;
            string skrivVinst = File.ReadAllText("vinstTot.txt");
            double skrivVinstTal = double.Parse(skrivVinst);

            Console.WriteLine($"Din totala vinst {skrivVinstTal} tryck \"ENTER\" för att gå vidare");
            Console.ReadLine();
            ...
            while (gameState == true)
            {
                ...
                Console.WriteLine($"Din totala vinst den här omgången {vinstVärde}");
            ...
            double vinstTotInt = skrivVinstTal + vinstVärde;

            string vinstTot = Convert.ToString(vinstTotInt);
            File.WriteAllText("vinstTot.txt", vinstTot);
